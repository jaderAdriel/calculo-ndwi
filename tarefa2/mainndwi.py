...
def save_polygons_to_shapefile(polygons, crs, year=None):
    ...
    satellite = ""
    date = ""
    for file_name in os.listdir(f"./{year}"):
...
def get_bands_name_by_landsat(year, band):
    ...
    return mapa.get(landsat).get(band)

def save_ndwi_raster(ndwi, meta, year):
    output_directory = f"./NDWI_Rasters/"
    if not os.path.exists(output_directory):
        os.makedirs(output_directory)
    output_path = os.path.join(output_directory, f"ndwi_{year}.tif")
    meta.update(dtype=rasterio.float32, count=1)
    with rasterio.open(output_path, 'w', **meta) as dst:
        dst.write(ndwi.astype(rasterio.float32), 1)
    print(f"NDWI raster saved for year {year} at {output_path}")



# Função principal para processar as imagens de um determinado ano
def process_landsat_ndwi(year, threshold=0.0):
    area_of_interest_path = "./area_of_interest/area.shp"
    aoi_gdf = gpd.read_file(area_of_interest_path)

    green_band_path = get_band_path(year,  get_bands_name_by_landsat(year, 'GREEN'))
    nir_band_path =  get_band_path(year,  get_bands_name_by_landsat(year, 'NIR'))
    ...
    save_ndwi_raster(ndwi, green_meta, year)



years = [1989, 1999, 2007, 2013]
